               ...
               <Link to='/' className="navbar-brand"><img className="agencyLogo" src={agencyLogo} alt=""/></Link>
                ...
                <Navbar.Collapse id="basic-navbar-nav" className='mt-5 pt-3'>
                    <Nav className="ml-auto">
                      <Link style={linkStyle} className="nav-link mr-4" to="/home">Home <span className="sr-only">(current)</span></Link>
                      <Link style={linkStyle}  className="nav-link mr-4" to="/portfolios">Our Portfolio</Link>
                      <Link style={linkStyle} className="nav-link mr-4" to="/ourTeam">Our Team</Link>
                      <Link style={linkStyle} className="nav-link mr-4" to="/contactUs">Contact Us</Link>
                      {isAdmin && <Link className="nav-link mr-4" to="/serviceList">Admin Dashboard</Link>}
                      {!(loginUser.email||loginUser.currentEmail || currentEmail ) && <button onClick={handleChange} className='login-btn mr-4 mt-1'>Login</button>}
                       {(loginUser.email||loginUser.currentEmail||currentEmail )&& <button onClick={handleLogoutUser} className='login-btn mr-4 mt-1'>Logout</button>}
                    </Nav>
                </Navbar.Collapse>
        </Navbar>
       </div>
