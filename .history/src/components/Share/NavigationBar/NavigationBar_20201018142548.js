    ...
    return (
       <div className="container">
              ...
              <Link to='/' className="navbar-brand"><img className="agencyLogo" src={agencyLogo} alt=""/></Link>
                <Navbar.Toggle aria-controls="basic-navbar-nav mt-5 pt-3" />
                <Navbar.Collapse id="basic-navbar-nav">
                    <Nav className="ml-auto">
                      <Link className="nav-link mr-3 mt-4" to="/home">Home <span className="sr-only">(current)</span></Link>
                      <Link className="nav-link mr-3 mt-4" to="/portfolios">Our Portfolio</Link>
                      <Link className="nav-link mr-3 mt-4" to="/ourTeam">Our Team</Link>
                      <Link className="nav-link mr-3 mt-4" to="/contactUs">Contact Us</Link>
                      {isAdmin && <Link className="nav-link mr-3" to="/serviceList">Admin Dashboard</Link>}
                      {!(loginUser.email||loginUser.currentEmail || currentEmail ) && <button onClick={handleChange} className='login-btn'>Login</button>}
                       {(loginUser.email||loginUser.currentEmail||currentEmail )&& <button onClick={handleLogoutUser} className='login-btn'>Logout</button>}
                    </Nav>
                </Navbar.Collapse>
        </Navbar>
       </div>
